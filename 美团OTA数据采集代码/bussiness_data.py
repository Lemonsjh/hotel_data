...
HOURLY_HEADERS = [
    ...
    "metric_code", "metric_name", "metric_value", "metric_unit", "competitor_rank", "peer_average",
]
...
def build_hourly_metric_rows(data, captured_at):
    rows = []
    metrics = data.get("metrics") or {}
    snapshot_hour = captured_at.replace(minute=0, second=0, microsecond=0)
    for metric_id, fallback_name in METRIC_ORDER:
        metric = metrics.get(metric_id) or {}
        rows.append([
            HOTEL_ID, data.get("score_hotel_name", ""), captured_at.date(), captured_at, snapshot_hour,
            metric_id, metric.get("name") or fallback_name, hourly_metric_value(metric), metric.get("unit", ""),
            metric.get("rank", "-"), metric.get("peer_avg", "-"),
        ])
    return rows


def save_hourly_snapshot(data, captured_at):
    rows = build_hourly_metric_rows(data, captured_at)
    write_standard_json(OUTPUT_DIR / "meituan_ota_business_metrics_hourly.json", HOURLY_HEADERS, rows)
    sync_meituan_business_metrics_hourly(HOURLY_HEADERS, rows)

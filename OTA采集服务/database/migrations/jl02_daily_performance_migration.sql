...
CREATE TABLE IF NOT EXISTS `jl02_hotel_performance_daily` (
    ...
    `room_type_name` VARCHAR(255) NOT NULL DEFAULT '' COMMENT '房型名称，非房型指标为空',
    ...
    UNIQUE KEY `uk_jl02_hotel_date_metric` (
        `hotel_id`, `business_date`, `category`, `room_type_name`, `metric_name`
    ),
    KEY `idx_jl02_room_type_id` (`hotel_id`, `room_type_id`),

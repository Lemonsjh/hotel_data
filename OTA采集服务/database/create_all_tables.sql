...
-- Table: bypms_channel_unit_mapping_snapshot
...
CREATE TABLE IF NOT EXISTS `rs01_room_revenue_daily` (
  ...
  `channel_unit_id` varchar(100) DEFAULT NULL COMMENT 'OTA渠道商品/售卖单元ID，PMS未提供时为空',
  `snapshot_time` datetime NOT NULL,
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  ...
  KEY `idx_order_id` (`order_id`),
  KEY `idx_channel_unit_id` (`hotel_id`,`channel_unit_id`),
  KEY `idx_snapshot_time` (`snapshot_time`),
  KEY `idx_hotel_room_type_id` (`hotel_id`,`room_type_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;

-- Table: ota_review_reply_task
CREATE TABLE IF NOT EXISTS `ota_review_reply_task` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_ota_review_reply_task` (`hotel_id`,`channel_source`,`review_id`),

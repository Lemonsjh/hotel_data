...
def main() -> int:
    ...
    with requests.Session() as session:
        ...
        master = fetch_room_master(session)
    master_types = {
        str(item.get("id")): str(item.get("name") or "").strip()
        for item in (master or {}).get("typeVos", []) if isinstance(item, dict) and item.get("id")
    }
    if not master_types:
        raise RuntimeError("ByPMS room master unavailable; mapping snapshot not replaced")
    sync_mysql(rows, snapshot_time.replace(minute=0, second=0, microsecond=0))
    with pymysql.connect(**mysql_config()) as connection:
        mapping_stats = bypms_auto_mapping.synchronize(
            connection, HOTEL_ID, HOTEL_NAME,
            {"meituan": os.environ.get("MEITUAN_HOTEL_NAME", ""),
            "ctrip": os.environ.get("CTRIP_HOTEL_NAME", "")}, master_types,
        )
    mapping_changes = sum(mapping_stats[key] for key in ("aliases", "bases", "renames", "products"))
    if mapping_changes:
        enrich_stats = room_type_enrichment.enrich_tables(
            {"mysql": mysql_config()}, hotel_id=HOTEL_ID,
        )
        print(f"ByPMS room type IDs filled: tables={len(enrich_stats)} "
              f"rows={sum(item['matched_by_product'] + item['matched_by_name'] for item in enrich_stats.values())}")
    ...
    print(f"ByPMS channel mapping synced: rows={len(rows)} mapping_delta={mapping_stats}")
    return 0

...
def load_session():
    ...
    payload_template = data.get("kf11_payload", {})
    api_url = data.get("kf11_api_url")

    if not api_url:
        print("❌ 没有 KF11 API（请先抓一次接口）")
        return None
...
def fetch_kf11(start_date=None, end_date=None):
    ...
    try:
        ...
        out_path = os.path.join(OUTPUT_DIR, "KF11.json")

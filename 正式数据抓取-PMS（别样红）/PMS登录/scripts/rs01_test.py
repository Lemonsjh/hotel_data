...
def load_session():
    ...
    payload_template = data.get("rs01_payload", {})
    api_url = data.get("rs01_api_url")

    if not api_url:
        print("❌ 没有 RS01 API（请先抓一次接口）")
        return None
...
def fetch_rs01(start_date=None, end_date=None):
    ...
    try:
        ...
        out_path = os.path.join(OUTPUT_DIR, "RS01.json")
